...
estadoCivil.all = async function (req, res) {
    try {
        let estadosCivis = await con.query("SELECT * FROM estadocivil;")
...
estadoCivil.create = async function(req, res) {
    try{
        let estado = req.body
        let sql = "INSERT INTO estadocivil (estado_nome) values (?);"
...
estadoCivil.update = async function(req,res){
    try{
        ...
        let sql = "UPDATE estadocivil SET nome=? WHERE id_estado_civil=?; "
        const values = [EstadoNovo.nome, Estado]
        let result = await con.query(sql, values)
        res.send({
